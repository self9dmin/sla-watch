...
- Opens on a compact Coverage workspace with provider, service, incident, and filing facts above a Smartscape-backed scope map. Manual coverage is a secondary path for services without a usable runtime relationship.
- Uses exact service-to-cloud-runtime relationships from Smartscape to suggest provider candidates. Suggestions are never created from service names and require operator confirmation before a mapping is stored.
...
- Lets an operator confirm app-owned coverage for an exact service that is outside the Scope map. The mapping can use provider-level terms or one provider service and can be updated or removed without modifying Dynatrace entity metadata.
...
3. Open **Coverage**, then use **Scope map** to review Smartscape recommendations and confirm the exact service-to-provider-service relationships used by Incidents and Evidence.
4. Use **Manual coverage** only for services without a usable Smartscape relationship. Choose provider-level terms or an exact provider service, then confirm the app-owned mapping. Existing source tags are recognized as read-only evidence and are not changed by the app.
...
7. Select **Review terms** to inspect the complete published provider record, service-level coverage, support options, and any tenant overrides before assessing an incident.
...
Confirmed coverage uses the `provider-scope-assignments` App Settings schema. A Scope map record contains exact Dynatrace service and runtime IDs, display context, the selected provider-service ID, and a short evidence note. A Manual coverage record contains the exact service ID and either provider-level terms or one selected provider service. Coverage is the app landing page and opens on Scope map. Incident review checks exact tenant contract scopes first, then confirmed coverage, then a unique Smartscape candidate. Any candidate remains visibly unconfirmed until an operator saves it in Coverage. Existing provider tags can contribute read-only evidence, but the app never writes or removes them.
