...
- Provides an Incidents workspace with a bounded, prioritized Dynatrace Problem worklist. Active Problems and exact provider-relevant candidates appear first, the selected summary shows only impact, root-cause state, provider match, and the next action, and full investigation opens in the native Problems app.
- Provides an Evidence workspace that assembles the Dynatrace Problem, exact affected-service request and failure telemetry, native objective posture, applicable terms, exclusions, and required evidence into a compact follow-up package. The SRE saves Package ready, Not ready, or Not provider-related. Provider reports remain optional supporting evidence, and nothing is submitted.
- Keeps the operating sequence inside one shell: Coverage, Performance, Incidents, then Evidence. Published and tenant-specific terms are available through Review terms. FinOps Agent is visible as planned and disabled until the platform supports the intended capability.
...
Human candidate decisions use the `evidence-decisions` App Settings schema. Each record stores a bounded Problem snapshot, exact affected entity IDs, the provider and mapping basis, the package-ready, not-ready, or not-provider-related decision, the acknowledged provider-evidence checklist, a concise operational note, and the review time. The app reuses a decision only while the mapping basis, affected entities, and provider-service scope still match the current candidate. These records are shared operational context. They do not establish root cause, provider liability, SLA eligibility, credit approval, or submission. Do not store confidential contract text, credentials, or personal data in a decision note.
