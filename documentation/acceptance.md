...
This record captures the manual target-environment smoke evidence for the `0.0.18` release candidate. It complements automated tests; it is not a substitute for authenticated least-privilege and Playwright acceptance jobs.
...
| Release artifact | The `0.0.18` manifest, icon, function, and UI deploy successfully through `dt-app` | `npx dt-app deploy --environment-url ...` completed successfully |
