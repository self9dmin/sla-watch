...
This record captures the manual target-environment smoke evidence for the `0.0.24` release candidate. It complements automated tests; it is not a substitute for authenticated least-privilege and Playwright acceptance jobs.
...
| Change log | The header and settings rail open a read-only release history; the release history links to the Dynatrace Community profile | Deployed browser smoke |
| Community support | The header support action and dashboard support action open the configured Dynatrace Community profile in a new tab | Deployed browser smoke |
| Release artifact | The `0.0.24` manifest, icon, function, and UI deploy successfully through `dt-app` | `npx dt-app deploy --environment-url ...` completed successfully |
