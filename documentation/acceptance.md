...
This record captures the manual target-environment smoke evidence for the `0.0.25` release. The release was deployed to the sal98008 Dynatrace environment and exercised through the installed application. This record complements automated tests and is not a substitute for least-privilege and Playwright acceptance jobs.
...
| Release artifact | The `0.0.25` manifest, icon, function, and UI pass the supported-runtime release gate and deploy successfully | `npm run verify:release`; `dt-app deploy` completed; installed change log reports `0.0.25 Current release` |
