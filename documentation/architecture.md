...
- Global service and preferred-anchor queries are intentionally bounded. Separate aggregate counts detect truncation, the UI refuses to claim complete coverage when a count cannot be verified, and recent Problem service IDs trigger sanitized incident-scoped topology, service, request, and failure queries. Processes and containers remain supporting incident evidence rather than primary Coverage rows.
...
- Incidents is deliberately not a second Problems app. It pages eight records, prioritizes active and provider-relevant Problems, and exposes the returned root-cause entity plus a conservative provider-relationship label. Exact same-root-cause groups may be suggested together. Manual multi-select also permits closed, confirmed, unresolved candidates without a returned root cause, while keeping that missing evidence visible through a second confirmation. Active Problems, selected-provider root links, incomplete context, and prior decisions remain ineligible. An incident-scoped topology overflow or a truncated settings read disables bulk mutation instead of treating missing data as absence. Full causal graphs, logs, traces, filters, and remediation stay in the native Problems app.
...
- Evidence decisions are shared App Settings records readable by authenticated app users. Bulk triage writes one record per Problem, never one aggregate decision, and reports partial failures. New records snapshot the root-cause entity and are invalidated when the Problem state or root-cause identity changes. Notes must remain concise and operational. They must not contain credentials, confidential contract text, personal data, or an assertion that the provider accepted liability or approved a credit.
