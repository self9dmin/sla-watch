...
SLA Watch is a read-mostly Dynatrace AppEngine application with one narrow, user-confirmed write path. It compares a selected provider contract from `sla.directory` with the live service inventory and recent telemetry visible to the current Dynatrace user. Its output is an evidence posture for human review, not an automated credit decision.
...
The application has two write paths:
...
6. Provider tags are written only to service entity IDs selected in Setup. Existing tags are preserved. Services with a conflicting provider tag are excluded from the bulk selection and require manual review.
...
- Provider service identifiers are not yet joined automatically to Dynatrace service entity IDs. The current provider boundary is tag-based and must remain a review aid, not proof of fault (`README.md`, `ui/app/pages/Dashboard.tsx`).
