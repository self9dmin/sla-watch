...
- App state services for user preferences and shared watch/workflow records.
...
```text
Settings, Bug management, Change management -> SlaPreferencesContext -> user/app state service
...
5. App state stores provider configuration, workflow records, and personal display preferences. It does not store credentials or Dynatrace tokens.
...
- Personal and shared workflow state: Dynatrace app-state services when available.
...
- Shared app-state writes are workspace-wide and scope-controlled. A user with write permission can change shared provider configuration and workflow records (`documentation/permissions.md`).
