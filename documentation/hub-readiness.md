...
| --- | --- | --- | --- |
| Manifest identity | `app.config.json` uses `my.sla`, `SLA Watch`, version `0.0.22`, a maintained icon, and a sub-80-character description | Implemented locally and deployed at `0.0.22` | Confirm the permanent publisher-owned app ID and final Hub name. Do not change the ID casually after distribution. |
...
| External API | `api/slaDirectory.function.ts` validates input, bounds time, validates the response, and preserves provider credit and filing fields | Implemented and confirmed connected in the `0.0.22` deployment | Re-verify `sla.directory` allowlisting after each environment install. |
...
| Themes | Custom styles use theme variables and dark/light paths were manually verified in the target tenant | Verified at `0.0.22` after deployment | Add automated contrast or screenshot coverage before submission. |
