...
| --- | --- | --- | --- |
| Manifest identity | `app.config.json` uses `my.sla`, `SLA Review`, version `0.0.42`, a maintained icon, and a sub-80-character description | `0.0.42` is deployed and verified in the designated target environment | Confirm the permanent publisher-owned app ID and final Hub name. Do not change the ID casually after distribution. |
...
| Themes | Custom styles use theme variables and dark/light paths were manually verified in the target tenant | The installed `0.0.42` Scope map, Manual coverage, and Evidence views were verified as compact and readable in light and dark themes. The original dark preference was restored | Add automated contrast or screenshot coverage before submission. |
...
| Tests | Focused unit tests, typecheck, lint, build, analyzer, and guarded one-screen browser assertions are present | The `0.0.42` release gate passes 18 suites and 103 tests. Production create, persistence, reuse, reset, removal, and cleanup passed; denied-permission evidence remains open | Add authenticated Playwright and denied/read-only acceptance jobs using CI secrets. |
| CI | Pull-request workflow is repository-local and non-deploying | Verified in [GitHub Actions run 34561366779](https://github.com/self9dmin/sla-watch/actions/runs/34561366779) on Node 24 for `ab0a671` | Make the required checks branch-protection rules and keep deployment outside the pull-request job. |
