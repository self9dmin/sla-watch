...
| --- | --- | --- | --- |
| Manifest identity | `app.config.json` uses `my.sla`, `SLA Review`, source version `0.0.48`, a maintained icon, and a sub-80-character description | `0.0.48` is deployed and verified in the target environment from source commit `b153ff6` | Confirm the permanent publisher-owned app ID and final Hub name. Do not change the ID casually after distribution. |
...
| Themes | Custom styles use theme variables and dark/light paths were manually verified in the target tenant | The direct-to-Coverage workflow has prior light and dark verification, and the installed `0.0.48` production surface passed the current dark-theme smoke | Add automated contrast or screenshot coverage before submission. |
...
| Tests | Focused unit tests, typecheck, lint, build, analyzer, and guarded one-screen browser assertions are present | Source `0.0.48` passes 20 suites and 126 tests plus typecheck, lint, build, analyzer, and production dependency audit. The deployed `0.0.48` release passed a connected Chrome smoke across Coverage, Incidents, Evidence, Directory, Settings, and change log with no captured browser warnings or errors. Live AWS affected-entity and denied-permission evidence remain open | Add authenticated Playwright and denied/read-only acceptance jobs using CI secrets. |
