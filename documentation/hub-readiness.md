...
| --- | --- | --- | --- |
| Manifest identity | `app.config.json` uses `my.sla`, `SLA Review`, version `0.0.45`, a maintained icon, and a sub-80-character description | `0.0.45` is the local release candidate; `0.0.44` remains the last deployed and verified target-environment release | Confirm the permanent publisher-owned app ID and final Hub name. Do not change the ID casually after distribution. |
...
| Tests | Focused unit tests, typecheck, lint, build, analyzer, and guarded one-screen browser assertions are present | The `0.0.45` local candidate passes 19 suites and 117 tests plus typecheck, lint, build, analyzer, and production dependency audit. Its exception-only queue, observed EC2 matches, search, and both themes passed tenant-hosted local review; denied-permission evidence remains open | Add authenticated Playwright and denied/read-only acceptance jobs using CI secrets. |
