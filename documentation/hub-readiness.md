...
| --- | --- | --- | --- |
| Manifest identity | `app.config.json` uses `my.sla`, `SLA Review`, version `0.0.42`, a maintained icon, and a sub-80-character description | `0.0.42` is the current release candidate | Confirm the permanent publisher-owned app ID and final Hub name. Do not change the ID casually after distribution. |
...
| Confirmed provider-service scope | Coverage stores operator-confirmed Smartscape or direct-service mappings in the `provider-scope-assignments` schema; Incidents and Evidence reuse currently valid exact mappings | `0.0.42` local release-candidate acceptance verified direct-service create, reload persistence, and removal without changing entity metadata; production and read-only acceptance remain open | Verify production create, update, remove, read-only access, and review reuse with a non-confidential mapping. |
| Human evidence decisions | Evidence derives candidates only from exact affected services that overlap a confirmed mapping, exact provider tag, or same-provider Smartscape suggestion. One explanation leads to Validate or Dismiss, and changed evidence boundaries invalidate stale decisions | Deployed in `0.0.41`; representative interaction and false-positive paths are covered, while the live tenant correctly withheld candidates when no Problem overlapped the AWS boundary. App Settings mutation acceptance remains open | Verify validate, dismiss, update, read-only access, and removal with non-confidential notes. Confirm every surface preserves the provider-fault and credit-eligibility boundary. |
...
| Themes | Custom styles use theme variables and dark/light paths were manually verified in the target tenant | The installed `0.0.41` shell and Evidence views were verified in light and dark themes. Coverage, Incidents, Evidence, Provider reports, and Directory retained normal desktop fit without body overflow | Add automated contrast or screenshot coverage before submission. |
...
| Tests | Focused unit tests, typecheck, lint, build, analyzer, and guarded one-screen browser assertions are present | The `0.0.42` local suite passes 18 suites and 103 tests; full release, production mutation, and denied-permission evidence are pending | Add authenticated Playwright and disposable-service acceptance jobs using CI secrets. |
| CI | Pull-request workflow is repository-local and non-deploying | Verified in [GitHub Actions run 34559101335](https://github.com/self9dmin/sla-watch/actions/runs/34559101335) on Node 24 for `13e1a74` | Make the required checks branch-protection rules and keep deployment outside the pull-request job. |
