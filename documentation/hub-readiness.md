...
| --- | --- | --- | --- |
| Manifest identity | `app.config.json` uses `my.sla`, `SLA Review`, version `0.0.39`, a maintained icon, and a sub-80-character description | `0.0.39` is the latest verified target-environment deployment | Confirm the permanent publisher-owned app ID and final Hub name. Do not change the ID casually after distribution. |
...
| Confirmed provider-service scope | Setup stores an operator-confirmed provider service against exact service/runtime IDs in the `provider-scope-assignments` schema; Overview and Incident review reuse currently valid exact mappings | Deployed in `0.0.37`; live App Settings mutation acceptance remains open | Verify create, update, remove, read-only access, and incident reuse with a non-confidential mapping. |
...
| Themes | Custom styles use theme variables and dark/light paths were manually verified in the target tenant | The `0.0.39` production dark view and tenant-hosted local light view were manually verified; prior installed coverage includes Scope map, Service tags, and provider connections | Add automated contrast or screenshot coverage before submission. |
...
| Tests | Focused unit tests, typecheck, lint, build, analyzer, and guarded one-screen browser assertions are present | The `0.0.39` release gate passed 16 suites and 93 tests; live mutation and denied-permission tests are gaps | Add authenticated Playwright and disposable-service acceptance jobs using CI secrets. |
