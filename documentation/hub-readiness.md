...
| --- | --- | --- | --- |
| Manifest identity | `app.config.json` uses `my.sla`, `SLA Watch`, version `0.0.24`, a maintained icon, and a sub-80-character description | Implemented locally; deployment pending for this release | Confirm the permanent publisher-owned app ID and final Hub name. Do not change the ID casually after distribution. |
...
| External API | `api/slaDirectory.function.ts` validates input, bounds time, validates the response, and preserves provider credit and filing fields | Implemented and confirmed connected in the `0.0.23` deployment; unchanged in `0.0.24` | Re-verify `sla.directory` allowlisting after each environment install. |
...
| Privacy | App-state TTL is 90 days, only provider configuration is persisted, and UI warns about local fallback | Implemented locally | Review operator guidance for PII and verify deletion/expiry behavior in a tenant. |
| Themes | Custom styles use theme variables and dark/light paths were manually verified in the target tenant | Verified at `0.0.23` after deployment; change log styles added for `0.0.24` | Add automated contrast or screenshot coverage before submission. |
