...
| --- | --- | --- | --- |
| Manifest identity | `app.config.json` uses `my.sla`, `SLA Review`, version `0.0.40`, a maintained icon, and a sub-80-character description | `0.0.40` is deployed and verified in the target environment | Confirm the permanent publisher-owned app ID and final Hub name. Do not change the ID casually after distribution. |
...
| Human evidence decisions | Evidence derives candidates only from exact affected services that overlap a confirmed mapping, explicit provider tag, or Smartscape suggestion; validate and dismiss decisions use the `evidence-decisions` schema | Deployed in `0.0.40`; the live tenant correctly withheld candidates when no Problem overlapped covered services, so App Settings mutation acceptance remains open | Verify validate, dismiss, update, read-only access, and removal with non-confidential notes. Confirm every surface preserves the provider-fault and credit-eligibility boundary. |
...
| Themes | Custom styles use theme variables and dark/light paths were manually verified in the target tenant | The installed `0.0.40` Coverage workspace was verified in light and dark themes. Incidents, Evidence, and Directory were verified in the installed light theme, and their dark paths were verified through the tenant-hosted local app | Add automated contrast or screenshot coverage before submission. |
...
| Tests | Focused unit tests, typecheck, lint, build, analyzer, and guarded one-screen browser assertions are present | The `0.0.40` release gate passed 18 suites and 100 tests; live mutation and denied-permission tests are gaps | Add authenticated Playwright and disposable-service acceptance jobs using CI secrets. |
| CI | Pull-request workflow is repository-local and non-deploying | Verified in [GitHub Actions run 34557378292](https://github.com/self9dmin/sla-watch/actions/runs/34557378292) on Node 24 for `dfa0a6e` | Make the required checks branch-protection rules and keep deployment outside the pull-request job. |
