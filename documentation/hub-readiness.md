...
| --- | --- | --- | --- |
| Manifest identity | `app.config.json` uses `my.sla`, `SLA Watch`, release-candidate version `0.0.37`, a maintained icon, and a sub-80-character description | The acceptance log retains `0.0.36` as the latest verified deployment until `0.0.37` is deployed and smoked | Confirm the permanent publisher-owned app ID and final Hub name. Do not change the ID casually after distribution. |
...
| Confirmed provider-service scope | Setup stores an operator-confirmed provider service against exact service/runtime IDs in the `provider-scope-assignments` schema; Incident review reuses it | Deployed in `0.0.36`; live App Settings mutation acceptance remains open | Verify create, update, remove, read-only access, and incident reuse with a non-confidential mapping. |
