...
| --- | --- | --- | --- |
| Manifest identity | `app.config.json` uses `my.sla`, `SLA Review`, version `0.0.46`, a maintained icon, and a sub-80-character description | `0.0.46` is the deployed and verified target-environment release | Confirm the permanent publisher-owned app ID and final Hub name. Do not change the ID casually after distribution. |
...
| Tests | Focused unit tests, typecheck, lint, build, analyzer, and guarded one-screen browser assertions are present | The deployed `0.0.46` release passes 19 suites and 121 tests plus typecheck, lint, build, analyzer, and production dependency audit. Direct Coverage startup and the optional walkthrough passed production smoke; both themes passed tenant-hosted local review. Denied-permission evidence remains open | Add authenticated Playwright and denied/read-only acceptance jobs using CI secrets. |
