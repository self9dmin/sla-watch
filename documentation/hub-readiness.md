...
| --- | --- | --- | --- |
| Manifest identity | `app.config.json` uses `my.sla`, `SLA Review`, source version `0.0.48`, a maintained icon, and a sub-80-character description | `0.0.46` remains the deployed and verified target-environment release until the prepared source is published | Confirm the permanent publisher-owned app ID and final Hub name. Do not change the ID casually after distribution. |
...
| Themes | Custom styles use theme variables and dark/light paths were manually verified in the target tenant | The direct-to-Coverage `0.0.46` release was verified locally in both themes and in the installed dark production surface | Add automated contrast or screenshot coverage before submission. |
...
| Tests | Focused unit tests, typecheck, lint, build, analyzer, and guarded one-screen browser assertions are present | Source `0.0.48` passes 20 suites and 126 tests plus typecheck, lint, build, analyzer, and production dependency audit. The deployed `0.0.46` release retains the last recorded production smoke. Live AWS affected-entity and denied-permission evidence remain open | Add authenticated Playwright and denied/read-only acceptance jobs using CI secrets. |
