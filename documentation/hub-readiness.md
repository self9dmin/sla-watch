...
| --- | --- | --- | --- |
| Manifest identity | `app.config.json` uses `my.sla`, `SLA Review`, version `0.0.41`, a maintained icon, and a sub-80-character description | `0.0.41` is deployed and verified in the target environment | Confirm the permanent publisher-owned app ID and final Hub name. Do not change the ID casually after distribution. |
...
| Confirmed provider-service scope | Coverage stores an operator-confirmed provider service against exact service/runtime IDs in the `provider-scope-assignments` schema; Incidents and Evidence reuse currently valid exact mappings | Deployed in `0.0.37`; live App Settings mutation acceptance remains open | Verify create, update, remove, read-only access, and review reuse with a non-confidential mapping. |
...
| Tests | Focused unit tests, typecheck, lint, build, analyzer, and guarded one-screen browser assertions are present | The `0.0.41` release gate passed 18 suites and 107 tests; live mutation and denied-permission tests are gaps | Add authenticated Playwright and disposable-service acceptance jobs using CI secrets. |
