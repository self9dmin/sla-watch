...
| --- | --- | --- | --- |
| Manifest identity | `app.config.json` uses `my.sla`, `SLA Review`, version `0.0.40`, a maintained icon, and a sub-80-character description | `0.0.40` is the current release candidate; `0.0.39` is the last recorded verified target-environment deployment | Confirm the permanent publisher-owned app ID and final Hub name. Do not change the ID casually after distribution. |
...
| Human evidence decisions | Evidence derives candidates only from exact affected services that overlap a confirmed mapping, explicit provider tag, or Smartscape suggestion; validate and dismiss decisions use the `evidence-decisions` schema | Implemented and unit tested in the `0.0.40` release candidate; live App Settings mutation acceptance remains open | Verify validate, dismiss, update, read-only access, and removal with non-confidential notes. Confirm every surface preserves the provider-fault and credit-eligibility boundary. |
...
| Themes | Custom styles use theme variables and dark/light paths were manually verified in the target tenant | The `0.0.40` Coverage landing page and Evidence empty/provider-report states were manually verified through the tenant-hosted local app in light and dark themes; installed production verification remains pending | Add automated contrast or screenshot coverage before submission. |
