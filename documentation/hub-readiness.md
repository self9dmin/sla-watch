...
| --- | --- | --- | --- |
| Manifest identity | `app.config.json` uses `my.sla`, `SLA Watch`, release-candidate version `0.0.35`, a maintained icon, and a sub-80-character description | The acceptance log records the last fully documented installed smoke separately; `0.0.35` remains a local release candidate until deployed and verified | Confirm the permanent publisher-owned app ID and final Hub name. Do not change the ID casually after distribution. |
...
| External API | The directory, Google health, and fixed public-status functions validate input, bound requests, validate responses, and preserve source boundaries | Directory access was confirmed in the recorded deployment; new public adapters remain release-candidate work | Re-verify every documented external host after each environment install and never disable allowlist enforcement. |
