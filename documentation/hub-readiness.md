...
| --- | --- | --- | --- |
| Manifest identity | `app.config.json` uses `my.sla`, `SLA Watch`, version `0.0.21`, a maintained icon, and a sub-80-character description | Implemented locally and deployed at `0.0.21` | Confirm the permanent publisher-owned app ID and final Hub name. Do not change the ID casually after distribution. |
...
| External API | `api/slaDirectory.function.ts` validates input, bounds time, and validates the response | Implemented and confirmed connected in the `0.0.21` deployment | Re-verify `sla.directory` allowlisting after each environment install. |
...
| Themes | Custom styles use theme variables and dark/light paths were manually verified in the target tenant | Verified at `0.0.21` after deployment | Add automated contrast or screenshot coverage before submission. |
...
The provider-service-to-Dynatrace-service join and automated credit eligibility calculation are intentionally outside this release. That limitation is disclosed in the README and UI. Implementing it is a product milestone, not a Hub-compliance shortcut.
