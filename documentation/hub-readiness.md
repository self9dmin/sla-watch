...
| --- | --- | --- | --- |
| Manifest identity | `app.config.json` uses `my.sla`, `SLA Review`, version `0.0.44`, a maintained icon, and a sub-80-character description | `0.0.44` is deployed and verified in the designated target environment | Confirm the permanent publisher-owned app ID and final Hub name. Do not change the ID casually after distribution. |
...
| Themes | Custom styles use theme variables and dark/light paths were manually verified in the target tenant | The unified `0.0.44` Coverage workflow was verified in light and dark during local tenant-hosted review, then verified in the installed dark production surface | Add automated contrast or screenshot coverage before submission. |
...
| Tests | Focused unit tests, typecheck, lint, build, analyzer, and guarded one-screen browser assertions are present | The `0.0.44` release gate passes 18 suites and 105 tests. The installed unified worklist, incident-priority order, exact custom-terms handoff, and no-scroll desktop fit passed; denied-permission evidence remains open | Add authenticated Playwright and denied/read-only acceptance jobs using CI secrets. |
