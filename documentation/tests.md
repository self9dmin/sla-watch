...
| Settings compatibility | Existing single-target custom records remain readable after the multi-target schema addition | A legacy `scopeEntityId` is normalized into the target list | `tests/contractOverrides.test.ts` | Existing unit test |
...
| Browser smoke | A deployed app exposes Overview, Setup, Incidents, Provider notices, and Directory in one task-ordered shell with a conservative posture | The Playwright suite visits each view, confirms the simplified global header, exercises the Directory term, service, support, and override views, opens the Setup scope map, and requires normal desktop states to fit without body scrolling | `tests/e2e/sla-watch.spec.ts`, `playwright.config.ts` | Prepared guarded E2E |
...
| Custom terms workflow | New records belong in Settings and require explicit evidence targets | Browser smoke confirms no create dialog is used, service scope begins empty, and selecting an exact target updates the assignment | `tests/e2e/sla-watch.spec.ts` | Prepared guarded E2E |
