...
| Header action guidance | Every icon-only header action must identify itself without relying on icon recognition | The Playwright suite hovers each header action and verifies its Strato tooltip, including the disabled Community state | `tests/e2e/sla-watch.spec.ts` | Prepared guarded E2E |
