...
| `sts.us-east-1.amazonaws.com` | AWS role assumption and account identity verification | Server-side outbound host | Fixed function constant and environment allowlist | AWS-managed endpoint; rotate the vaulted base credential separately | High: creates request-local role sessions and prevents a connection from reading the wrong AWS account |
...
| `contract-overrides` | Custom terms and exact Dynatrace targets | Environment-shared App Settings | Authorized user input | Persists until updated or removed; Settings keeps version history | High: changes which terms are applied during review |
| `provider-connections` | Provider, account, subscription, project, or tenancy scope; optional AWS role ARN; OCI region; Credential Vault ID; and enabled state | Environment-shared App Settings | Authorized administrator input | Persists until updated or removed | High: selects which provider scope is queried, but contains no secret value |
| `provider-scope-assignments` | Operator-confirmed SLA match for an exact Dynatrace service or service-to-runtime relationship | Environment-shared App Settings | Authorized operator decision in Coverage | Persists until updated or removed; Settings keeps version history | High: takes precedence over observed topology and controls which provider-service terms Incidents and Evidence apply |
| `evidence-decisions` | Per-Problem package-ready, not-ready, or not-provider-related review state plus acknowledged evidence items | Environment-shared App Settings | Authorized SRE decision in Evidence or guarded Incidents bulk review | Persists until reopened or replaced; Settings keeps version history | High: a case action writes one auditable record per supporting Problem and must not be represented as an SLA violation, provider fault, credit approval, or claim submission |
...
No API token, OAuth secret, password, private key, or private endpoint is bundled in the UI or function source. The `sla.directory`, Google Cloud Status, OCI Status, OpenAI Status, Anthropic Status, and ElevenLabs Status endpoints are public. AWS account access uses a vaulted base signing credential and can assume a dedicated Health role on every request. The resulting one-hour session remains only in function memory. Azure subscription access uses a dedicated client credential, personalized Google Cloud access uses a dedicated service-account JSON key, and OCI tenancy access uses a dedicated API user's signing key. Each durable secret stays only in Dynatrace Credential Vault. App Settings stores the Credential Vault record ID, non-secret provider scope, and optional AWS role ARN, not a credential value. Never add provider credential contents to `app.config.json`, App Settings, app state, a dashboard, a notebook, a test fixture, or the repository.
...
- Confirm each AWS Health role is restricted to the documented Health read actions, each vaulted base identity can assume only the intended role, and the effective account is verified with STS. Confirm the account has a supported AWS Health API plan. Do not vault a one-hour AssumeRole response as the durable base credential.
...
- Confirm custom records contain only operational terms, exact target IDs, effective dates, and a concise agreement reference. Confirm evidence decisions contain only bounded incident context, published checklist labels, and a concise operational note. Do not store confidential agreement text, credentials, or personal data because all authenticated app users can read App Settings.
- Confirm App Settings read and write policies are separated and that a read-only responder cannot mutate `contract-overrides`, `provider-scope-assignments`, or `evidence-decisions`.
