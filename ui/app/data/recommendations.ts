...
export const buildSetupRecommendations = ({
  ...
  if (directoryError || !directoryData) {
    add(recommendations, {
      ...
      detail: "SLA Watch cannot compare Dynatrace evidence with a contractual target until the selected sla.directory record is available.",
  ...
  if (directoryData && services.length > 0 && providerLabels.length === 0) {
    add(recommendations, {
      ...
      detail: `Add ${providerLabelKey}:${selectedProviderSlug} (or a documented equivalent) to the services that depend on ${directoryData.provider.name}. Names alone are not enough for a credit decision.`,
    ...
    add(recommendations, {
      ...
      detail: `Dynatrace has provider labels, but none resolve to ${directoryData.provider.name}. Confirm the intended provider or update the service tags before reviewing eligibility.`,
