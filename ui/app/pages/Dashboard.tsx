...
export const Dashboard = ({ initialSection = "overview" }: DashboardProps) => {
  ...
  return (
    <div className="dashboard-shell">
      ...
      {section === "overview" ? (
        <>
          <div className="metric-grid"><MetricCard label="Providers" value={directoryLoading ? "..." : directoryData ? "1" : "Unavailable"} detail={directoryData ? `${directoryData.provider.name} contract loaded` : "SLA.directory provider records"} tone={directoryData ? "positive" : directoryLoading ? "neutral" : "warning"} /><MetricCard label="Services in tenant" value={servicesLoading ? "..." : String(services.length)} detail="Live entity inventory" tone={servicesLoading ? "neutral" : services.length > 0 ? "positive" : "warning"} /><MetricCard label={`Problems · ${lookbackHours}h`} value={problemsLoading ? "..." : String(problems.length)} detail={problemsLoading ? "Reading Davis data" : `${activeProblems} active · ${problems.length - activeProblems} closed`} tone={problemsLoading ? "neutral" : activeProblems > 0 ? "warning" : "neutral"} /><MetricCard label="Claim readiness" value={claimReadiness.value} detail={claimReadiness.detail} tone={claimReadiness.tone} /></div>

          <div className="content-grid"><Surface className="panel-card connector-card"><Text className="eyebrow">Connection contract</Text><Heading level={3}>Make the directory the ownership source of truth.</Heading><Paragraph>{slaDirectoryConnection.message}</Paragraph><div className="contract-list"><div><span>Provider</span><strong>{directoryData?.provider.name ?? providerSlug}</strong></div><div><span>Directory transport</span><strong>Versioned JSON API</strong></div><div><span>Dynatrace evidence</span><strong>Services, Problems, logs, spans</strong></div></div><Button variant="default" onClick={() => void refetchDirectory()} disabled={directoryLoading}>{directoryLoading ? "Testing" : "Refresh provider"}</Button></Surface><Surface className="panel-card"><Text className="eyebrow">Next watch signal</Text><Heading level={3}>{claimReadiness.value === "Candidate" ? "A human review can start." : providerState.title}</Heading><Paragraph>Service health is a symptom. The provider decision needs the service boundary, a matching contract, and enough customer-side evidence to rule out a local cause.</Paragraph><div className="signal-row"><StatusPill tone={telemetryState.tone}>{telemetryState.title}</StatusPill><Text>{services.length} services · {problems.length} Problems</Text></div><div className="signal-row"><StatusPill tone={providerState.tone}>{providerState.title}</StatusPill><Text>{providerLabels.length} provider labels detected</Text></div><Link className="inline-action" to={providerState.tone === "warning" ? "/settings/watch" : "/bugs"}>{providerState.tone === "warning" ? "Fix the provider boundary" : "Report a mismatch"}</Link></Surface></div>
